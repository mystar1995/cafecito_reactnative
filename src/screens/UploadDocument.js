...
import {submitdocument} from '../service/userservice'
import Loading from 'react-native-loading-spinner-overlay'
export default function UploadDocument({navigation,route})
{
    ...
    const uploaddocument = (profile,iddocument) => {
        setloading(true)
        submitdocument(profile,iddocument,email).then(res=>{
            if(res.data.success)
            {
                setmodal(true)
            }
    ...
    const submit = () => {
        ...
        {
            uploadref.current.uploaddocument()
        }
    }
    ...
    return (
        <View style={style.container}>
            ...
            <ScrollView style={{flex:1}} showsVerticalScrollIndicator={false}>
                {
                    index < 2? <Guide index={index}/>:<Upload uploaddocument={uploaddocument} ref={uploadref}/>
                }
            ...
            <Modal 
                ...
                <View style={style.modalinside}>
                    ...
                    <TouchableOpacity style={[style.btncontainer,{width:wp('34.54'),justifyContent:'center'}]} onPress={()=>{setmodal(false);navigation.navigate('Login')}}>
