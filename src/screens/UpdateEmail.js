...
export default function UpdateEmail({navigation})
{
    ...
    const updateemail = () => {
        updateuserinfo({email:email},token).then(res=>{
            if(res.data.success)
            {
                dispatch(setprofileinfo({email:email}))
                ...
                <View style={{marginBottom:24}}>
                    ...
                    <View style={{marginTop:24,marginBottom:48}}>
                        ...
                        <View style={style.inputcontainer}>
                            <TextInput style={style.input} value={email} onChangeText={value=>setemail(value)}></TextInput>
                        </View>
                        <TouchableOpacity style={style.save_btn} onPress={updateemail}>
