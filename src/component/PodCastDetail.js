...
export default function PodCastDetail({id,image,title,author,tags,navigation})
{
    const {role} = useSelector(state=>state.auth)
    return (
        <TouchableOpacity style={style.container} onPress={()=>navigation.navigate('PodcastDetail',{id:id})}>
            <Image source={{uri:image}} style={{width:wp('23.18'),height:wp('23.18'),borderRadius:14}}></Image>
            <View style={{marginLeft:15,flex:1}}>
                <Text style={style.title}>{title}</Text>
                <Text style={style.description}>{author}</Text>
                {
                    tags && (
