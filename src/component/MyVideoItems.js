...
export default function MyVideoItem({video})
{
    return (
        <View style={{width:wp('23.18'),marginRight:14}}>
            <ImageBackground source={{uri:config.apiurl + "/" + video.coverimage}} style={{width:wp('23.18'),height:wp('23.18'),borderRadius:14,display:'flex',justifyContent:'center',alignItems:'center',position:'relative',overflow:'hidden'}}>
                ...
                <View style={{display:'flex',position:'absolute',bottom:5,right:5,flexDirection:'row',alignItems:'center'}}>
                    ...
                    <Text style={{color:'#F6AA11',fontSize:RFValue(9,580),fontFamily:'Quicksand-Medium'}}>{video.likes}</Text>
                </View>
            </ImageBackground>
